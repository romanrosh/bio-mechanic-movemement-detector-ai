...
def pose_estimation(file, estimation_dir):
    """
    :param file: name of the picture to estimate
    :return: save a new image and deletes the old one
    """
    ...
    cv2.imwrite(estimation_dir + file + '_points', frameCopy)
    cv2.imwrite(estimation_dir + file + '_skeleton', frame)
...
def pose_estimation_all(origin_dir,estimation_dir):
    "will convert all the pictures taken using the webcam to pose and point estimation"
    if not os.path.exists(estimation_dir):
        os.makedirs(estimation_dir)
    image_list = os.listdir(origin_dir)
    for image in image_list:
        pose_estimation(image, estimation_dir)
